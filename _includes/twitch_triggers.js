...
function LBTestTriggers() {
  const processTrigger = {
    LBTestTwitchSubs(form, notUsed, gifted = false, gifterName = '') {
      const type = 1;
      const subtype = (form.subgift.checked) ? 2 : (form.anongift.checked) ? 4 : gifted ? 2 : 1;
      const context = (form.subgift.checked) ? 'subgift' : (form.anongift.checked) ? 'anonsubgift' : gifted ? 'subgift' : 'resub';
      const month = parseInt(form.submonths.value) || 1;
      const name = gifted ? gifterName : form.anongift.checked ? 'Anonymous User' : generateName();
      ...
      let selectedTier; let selectedTierD;
      for (const tier of tiers) {
        if (tier.checked) {
          selectedTier = tier.value;
        }
      }
      const selecterTierNum = selectedTier === 'Tier 1' ? 1 : selectedTier === 'Tier 2' ? 2 : selectedTier === 'Tier 3' ? 4 : 8;
      const msg = (subtype !== 1) ? `${name} gifted a sub to ${giftedName}!` : `${name} subscribed for ${month} months!`;
      const data = {
        tier: selecterTierNum, month, subtype, communitygift: gifted ? 1 : 0,
      };
      const pullData = {
        user_name: name.toLowerCase(), display_name: name, user_id: 123456789, gifted_user_name: giftedName.toLowerCase(), gifted_display_name: giftedName, gifted_user_id: 123456789, tier: selectedTier, context, message, month, community_gift: gifted ? 1 : 0,
      };
    ...
    LBTestTwitchSubGift(form) {
      ...
      const selecterTierNum = selectedTier === 'Tier 1' ? 1 : selectedTier === 'Tier 2' ? 2 : selectedTier === 'Tier 3' ? 4 : 1;
      const gifterName = subForm.anongift.checked ? 'Anonymous user' : generateName();
      const amount = parseInt(form.subGiftAmount.value) || 1;
      const pullData = {
        user_name: gifterName.toLowerCase(), display_name: gifterName, user_id: 123456789, amount, tier: selectedTier,
      };
      sendTriggerToLB(2, `${gifterName} has gifted ${amount} subs!`, { tier: selecterTierNum, amount }, pullData);
    ...
    LBTestTwitchBits(form, pullData) {
      ...
      pullData.addvalues({ amount, total_amount: totalAmount, message });
      sendTriggerToLB(5, `${pullData.user_name} donated ${amount} bits!`, { amount }, pullData);
    },
    LBTestTwitchPoints(form, pullData) {
      ...
      const message = (userInput) ? form.channelPointsMsg.value || generateMessage() : undefined;
      ...
      pullData.addvalues({
        channel_id: channelID, redeem_name: redeemName, message, cost, image, reward_id: rewardId, redeem_id: redeemId,
      });
      sendTriggerToLB(5, `${pullData.display_name} has redeemed ${redeemName}!`, { redeemname: redeemName, message }, pullData);
    },
    LBTestTwitchRaid(form, pullData) {
      const amount = parseInt(form.raidAmount.value) || 5;
      pullData.addvalues({ amount });
      sendTriggerToLB(4, `${pullData.display_name} is raiding you with ${amount} viewers!`, { amount }, pullData);
    },
    LBTestTwitchHost(form, pullData) {
      const amount = parseInt(form.hostamount.value) || 5;
      pullData.addvalues({ amount });
      sendTriggerToLB(14, `${pullData.display_name} is hosting you with ${amount} viewers!`, { amount }, pullData);
    },
    LBTestTwitchChat(form) {
      ...
      if (form.chatSub.checked) {
        ...
        let month = (form.chatMsgSubMonth.value != 1) ? parseInt(form.chatMsgSubMonth.value) : 0;
      ...
      const pullData = {
        user_name: name.toLowerCase(), display_name: name, user_id: userId, message, emote_list: emoteList, badge_list: badge, channel, name_color: color, first_time: firstTime,
      };
      LB.trigger(0, { message, broadcaster: form.chatBroadcaster.checked, moderator: form.chatMod.checked, sub: form.chatSub.checked, vip: form.chatVip.checked, founder: form.chatFounder.checked, trigger_data: pullData,
      });
    },
    LBTestTwitchFollow(form, pullData) {
      sendTriggerToLB(6, `${pullData.display_name} followed you!`, {}, pullData);
    },
  };
  ...
  class ConstructPullData {
    constructor(type) {
      ...
      this.user_id = (type !== 'LBTestTwitchHost') ? Math.floor(Math.random() * 1000000000) : undefined;
  ...
  function generateName(name = '') {
    const names = ['Lioran', 'nutella4eva', 'Kottpower', 'Neverwho', 'Deomew', 'Doodybeard', 'Exeldro', 'AuralMoral', 'SuperMetroid', 'Melonax', 'Ptoil', 'LinksDarkHalf', 'Derpiii_', 'CoD360_420', 'Bill', 'Kurocha', 'Lisa', 'DeathByButtonMash', 'Daryl', 'Sebas', 'Ramsreef', 'Kelvin214', 'wellzish', 'Andilippi', 'MrWaldo', 'MrRubberDucky', 'Cyanidesugar', 'MisterK', 'Silverlink'];
  ...
  function sendTriggerToLB(type, message = 'Test trigger fired.', data = {}, triggerData) {
