...
function getCurrentExtensions() {
  ...
  tabs.forEach((tab) => {
    ...
    if (['twitch triggers', 'youtube triggers', 'extensions', 'status'].includes(tabTitle) || tabTitle === '') return;
...
async function populateExtensionTable() {
  ...
  currentExtensions.forEach((current) => {
    const latest = extensionData.find((e) => e.extension_name.toLowerCase().trim() === current.extension_name.toLowerCase().trim());
